<template>
  <v-card elevation="2">
    <div class="table-heading">
      <div>
        <p class="fw-600 mb-0">In-Process NOC Applications</p>
      </div>
      <div>
    ...
    <v-data-table v-model:sort-by="sortBy" :headers="headers" :items="applications" :loading="loading" :search="search" item-value="registrationId" density="comfortable">
      <template #no-data>
        <div class="text-center text-grey-darken-1 py-6">No in-process applications found.</div>
      </template>
    </v-data-table>
  </v-card>
</template>
...
import { getInprocessApplications } from '../services'
...
async function loadInprocessApplications() {
  loading.value = true
  try {
    applications.value = await getInprocessApplications()
  } finally {
    loading.value = false
  }
}
