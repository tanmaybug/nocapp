<template>
    ...
    <v-col cols="12" md="8" lg="9" class="d-flex flex-column">
      <v-card class="pa-6" elevation="2" rounded="lg">
        <div class="d-flex align-start justify-space-between ga-4">
          <div>
            <p class="font-weight-medium text-grey-darken-1 mb-2">Current Status</p>
            <v-chip color="info" size="large" variant="tonal" class="text-none font-weight-medium">
      ...
      <v-card class="pa-6 mt-4" elevation="2" rounded="lg">
        <div class="d-flex align-start justify-space-between ga-4">
          <div>
            <p class="font-weight-medium text-grey-darken-1 mb-2">Inspection Date</p>
            <v-chip :color="dashboardData.inspectionDate ? 'info' : 'warning'" size="large" variant="tonal" class="text-none font-weight-medium">
    ...
    <v-col cols="12" md="4" lg="3" class="d-flex">
      <v-card class="pa-4 flex-grow-1" elevation="2" rounded="lg">
        <div class="d-flex align-center justify-space-between mb-3">
          <p class="font-weight-medium mb-0">Quick Actions</p>
  ...
  <v-card class="mt-6" elevation="2">
    <div class="d-flex flex-wrap align-center justify-space-between pa-6 ga-4">
      <div>
        <p class="font-weight-medium mb-0">Recent Activities</p>
    ...
    <v-data-table v-model:sort-by="activitySortBy" :headers="activityHeaders" :items="dashboardData.activities" density="comfortable" item-value="sno" hide-default-footer>
      <template #item.sno="{ item }">
        <span class="font-weight-medium">{{ item.sno }}</span>
      </template>

      <template #item.activity="{ item }">
        <p class="font-weight-medium">{{ item.activity }}</p>
      </template>
