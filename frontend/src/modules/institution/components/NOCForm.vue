<template>
  <v-container>
    <v-card>
      <v-card-title class="text-center my-2">NOC Application Form</v-card-title>
      <v-card-text>
        <v-form @submit.prevent="onSubmitTab" ref="formRef">
          <v-tabs v-model="activeTab" color="primary" grow>
            <v-tab v-for="tab in tabs" :key="tab.value" :value="tab.value" :class="{ 'tab-submitted': store.isTabSubmitted(tab.dataKey) }">
          ...
          <v-window v-model="activeTab">
            <v-window-item v-for="tab in tabs" :key="tab.value" :value="tab.value">
              <component :is="tab.component" :model-value="getTabData(tab)" @update:model-value="updateTabData(tab, $event)" :validation="validation" />
            </v-window-item>
          </v-window>
          ...
          <v-row class="mt-5">
            <v-col cols="12" class="d-flex justify-space-between align-center">
              ...
              <div>
                ...
                <v-btn v-if="isLastTab && store.allTabsSubmitted" color="success" variant="flat" @click="onSubmitComplete" :loading="store.status === 'processing'">
                  Complete Submission
                </v-btn>
              </div>
            </v-col>
...
import { NOC_FORM_LABEL_MAP, useNOCStore, type NOCFormType } from '@/modules/institution/stores'
...
const currentTab = computed(() => tabs.find((tab) => tab.value === activeTab.value))
...
const getTabData = (tab: TabConfig) => store.getTabData(tab.dataKey)
const updateTabData = (tab: TabConfig, newValue: any) => {
